...
class TemporalConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # input size: [batch, nodes, time, features]
        identity = x
...
class ExpertHybridMechanism(nn.Module):
    ...
    def forward(self, x):
        ...
        # [B, num_experts] -> [B, 1, 1, 1, num_experts]

        weights = router_weights.unsqueeze(1).unsqueeze(1).unsqueeze(1)
        
        # F_expert_gelu = sum(W_router[l] * F_expert_l)
...
Multi-scale Conv1D + Feature Fusion + Router + Expert Hybrid
...
class GNN(nn.Module):
    ...
    def forward(self, h, A):
        ...
        for i, (s_conv, t_conv) in enumerate(zip(self.spatial_convs, self.temporal_convs)):
            ...
            h_spatial = torch.einsum('nkld,nkj->njld', h, A)  
            h_spatial = s_conv(h_spatial) 
            h_temporal = t_conv(h)
...
class MTGFLOWZL(nn.Module):
    ...
    def forward(self, x):
        ...
        total_loss = log_prob.mean() - 0.1 * contrastive_loss  
    ...
    def test(self, x, ):
        ...
        h_gcn, router_weights = self.gcn_mlp(h_gcn) 
        ...
        with torch.no_grad(): 
            ...
            z_cont = self.proj_cont_momentum(h_gcn_reshape)  
